...
public abstract class MyUnitClass {
    ...
    void search(Location loc) {
        ...
        if (uc.canMove(dir)) {
//            if (isAdjacentToAnotherUnit(currentLocation.add(dir))) moveRandomly();
            uc.move(dir);
            return;
        } else {
    ...
    void rush(){
        ...
        Location currentLocation = uc.getLocation();
        Location opponentBaseLocation = uc.getInitialLocation(opponent);
        Direction dir = currentLocation.directionTo(opponentBaseLocation);
    ...
    void moveToBarricade(Integer distance){
        Team team = uc.getTeam();
        Location currentLocation = uc.getLocation();
        Location homeBaseLocation = uc.getInitialLocation(team);
        Integer distanceFromHome = currentLocation.distanceSquared(homeBaseLocation);

        if (distanceFromHome < distance) {
            moveRandomlyAdjacent();
        }
    }

    void sweepPastBarricade(Integer distance){
        Team team = uc.getTeam();
        Location currentLocation = uc.getLocation();
        Location homeBaseLocation = uc.getInitialLocation(team);
        Integer distanceFromHome = currentLocation.distanceSquared(homeBaseLocation);

        Integer limit = distance + 4;
        if (distanceFromHome < limit){
            moveRandomly();
        }
